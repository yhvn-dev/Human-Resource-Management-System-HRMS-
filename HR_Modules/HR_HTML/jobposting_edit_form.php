...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!isset($_POST['jp_id']) || empty($_POST['jp_id'])) {
        die("Job post ID is required.");
    }

    // Sanitize and assign form data
    $jobpost_id = intval($_POST['jp_id']);
    ...
    $min_age = intval($_POST['jp_inp_job_min_age']);
    $max_age = intval($_POST['jp_inp_job_max_age']);
    $job_location = trim($_POST['jp_inp_job_location']);
    $salary_range_from = intval($_POST['jp_inp_salary_range_from']);
    $salary_range_to = intval($_POST['jp_inp_salary_range_to']);
    ...
    $contact_information = trim($_POST['jp_inp_contact_information']);
      ...
      $age_errors = validate_age_range((int)$min_age, (int)$max_age);
      if (!empty($age_errors)) {
          foreach ($age_errors as $error) {
              $age_comparison_errors[] = $error;
          }
      }



      if ($errors || $age_comparison_errors) {
          session_start();
          $_SESSION["jobpost_form_errors"] = $errors;
          $_SESSION["jobpost_age_errors"] = $age_comparison_errors;
          ...
          $SESSION["id"] = $jobpost_id;
          header("Location: ../HR_HTML/jobposting_edit_form.php?id=$jobpost_id");
          die();
      }
    ...
    if (!empty($errors)) {
        $_SESSION["jobpost_form_errors"] = $errors;
        $_SESSION["jobpost_form_data"] = $_POST;
    } else {
        // Update the job post with the new values
        $query = "UPDATE jobposts_ 
                  SET 
                      job_title = :job_title,
        ...
        $result = $stmt->execute([
            ...
            ':salary_range_to' => $salary_range_to,
            ':employment_type' => $employment_type,
            ':req_qual' => $required_qualifications,
            ':pref_qual' => $preferred_qualifications,
            ':appl_deadline' => $application_deadline,
            ':posting_date' => $posting_date,
            ':contact_information' => $contact_information,
            ':id' => $jobpost_id,
        ]);
        ...
        if ($result) {
            $_SESSION["jobpost_form_success"] = ["Job post updated successfully"];
            header("Location: jobposting_module.php?status=jobpost_updated_successfully");
            exit;
        } else {
            $errors[] = "Failed to update the job post. " . implode(" ", $stmt->errorInfo());
        }
    }
}
...
<head>
    ...
    <link rel="stylesheet" href="../HR_CSS/jobposting_edit.css">   
                    ...
                    <div class="jpf_container jpf_header">
                        ...
                        <div class="jp_left_header">
                            ...
                            <div class="lh_buttons_div">
                                ...
                                <a href="jobposting_module.php" class="lh_buttons_href" id="jpf_table">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="sb_button_icons" 
                        ...
                        <div class="jpf_container jpf_main_form" id="jpf_main_form_id">
                            ...
                            <div class="form_title_div">
                                ...
                                <span class="form-title" id="joblistform-lbl">Job List Edit Form</span>
                            ...
                            <form class="jp_form" method="POST" action="" enctype="multipart/form-data">
                                ...
                                <div class="edit_jp_form">
                                    ...
                                    <div class="jpf_button_div">
                                        ...
                                        <button type="submit" class="buttons" id="jpf_edit_button">Save</button>
